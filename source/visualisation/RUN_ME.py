...
f1.set_ylim(-5000, 11000)
f2.set_ylim(-9000, 9000)
f3.set_ylim(0, 255)
...
def animate(i):
  ...
  #cursor.execute('SELECT * FROM esp WHERE id=?', (i%584,))
  #data = cursor.fetchall()
  if data == []: return p1, p2, p3, tp1, tp2, p71, p72, p73, tts, tbp

  #print(data)

  data = data[0]

  d = {'time':data[2], 'ax':data[3], 'ay':data[4], 'az':data[5], 'gx':data[6], 'gy':data[7], 'gz':data[8], 'emg':data[9]}


  cursor.execute('SELECT * FROM opencv WHERE id=?', (i%438,))
  data = cursor.fetchall()[0]

  dcv = {'x':data[2],'y':data[3]}

  # if type(dcv['x']) == type('1'): return p1, p2, p3, tp1, tp2, p71, p72, p73, tts, tbp
